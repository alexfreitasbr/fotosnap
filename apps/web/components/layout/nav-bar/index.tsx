...
export function NavBar() {
  ...
  const menuItemsRender = NAVIGATION_ITEMS.map((item) => (
    <GoTo
      ...
      test-id={item.id}
    >
      <li className={`font-semibold ${isMenuOpen || windowWidth <= 640 ?  " text-2xl" : "text-lg whitespace-nowrap" }`}>{language.navigation[item.id]}</li>
    </GoTo>
  ));
    ...
    <>
      ...
      >
        ...
        {!isMenuOpen || windowWidth > 640 ? (
          <ul className="hidden w-full gap-4 sm:flex">
            {menuItemsRender}
          </ul>
        ) : (
